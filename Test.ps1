...
dotnet test --no-ansi --no-progress --configuration $Configuration --no-build
...
if ($IsWindows) {
  dotnet test --no-ansi --no-progress --root-directory $PSScriptRoot --test-modules "temp\**\Bshox.Tests.exe"
}
else {
  dotnet test --no-ansi --no-progress --root-directory $PSScriptRoot --test-modules "temp\**\Bshox.Tests"
}
