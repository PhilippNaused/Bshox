...
public class BshoxOptionsTests
{
    ...
    public async Task DefaultValue()
    {
        ...
        await Assert.That(BshoxOptions.Default.ToString()).IsEqualTo("BshoxOptions { MaxDepth = 64, LittleEndian = False, BufferSize = 16384 }");
    }
    ...
    public async Task ValidDepth()
    {
        ...
        await Assert.That(options.ToString()).IsEqualTo("BshoxOptions { MaxDepth = 1, LittleEndian = False, BufferSize = 16384 }");
    }
}
